...
export default function Dashboard() {
  ...
  const handleDownload = async (id, name) => {
    try {
      ...
      alert('Failed to download file securely: ' + (e.response?.data?.error || e.message));
    }
  };
        ...
        <div className="glass-panel" style={{ padding: '40px' }}>
            ...
            {loading ? <p>Synchronizing data streams...</p> : (
                ...
                <tbody>
                    ...
                    {data.logs.length === 0 && <tr><td colSpan="3" style={{ textAlign: 'center', padding: '64px', color: 'var(--text-muted)' }}>Waiting for telemetry...</td></tr>}
                </tbody>
                </table>
            )}
        </div>
        ...
        {loading ? <p>Loading sensor data...</p> : (
            ...
            <tbody>
                {data.files.map((file, i) => {
                  const displayName = file?.filename || 'Unnamed file';
                  const downloadName = displayName.replace(/^\d+-/, '');
                  const fileId = file?.id || file?._id;
                  const canDownload = file.status === 'clean' && !!fileId && !!file?.filename;
                  return (
                <tr key={i}>
                    ...
                    {canDownload ? (
                        ...
                        className="btn"
                        style={{ padding: '6px 14px', fontSize: '11px', border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '6px' }}
                        >
                        <Download size={14} /> Download
                        </button>
                    ) : (
                        <span style={{ color: 'var(--danger)', fontSize: '12px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '1px' }}>
                          {file.status === 'clean' ? 'Unavailable' : 'Quarantined'}
                        </span>
                    )}
                    </td>
                </tr>
